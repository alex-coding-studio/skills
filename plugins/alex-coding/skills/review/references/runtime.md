...
The selected complexity and model are stored per PR, included in checkpoints and returned by status; every exact-session resume uses them. Repeating `start` cannot silently change an existing choice. Review states and checkpoints created before model pinning keep their previous model selection (`gpt-5.6-sol` / `gpt-5.6-luna` or `claude-opus-5` / `claude-sonnet-5`) until explicit continuation; a fresh continuation selects the current model for its runtime and complexity. A newly discovered risk can require promotion: retain review progress and use the existing explicit continuation procedure rather than resetting rounds or switching an active session behind its owner. A continuation may supply `--complexity high`. Legacy checkpoints without a complexity choice use high. An unavailable model stops with retained evidence; it never triggers an automatic cheaper substitute.
