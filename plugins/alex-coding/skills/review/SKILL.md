...
Rounds belong to the PR and survive session replacement. At the escalation boundary, publish findings and a useful handoff before stopping. Publication failure retains the exact pending result; `retry` only retries publication. When the user explicitly requests a fresh review, Implement/Plan may use `continue --decision-file` to start a new independent session and extend the budget without resetting cumulative rounds. The old phase or unpublished result never blocks that request: the runner stops its verified previous process and preserves its evidence for the new reviewer. Author authorization to request review is not authorization to choose its verdict.
