...
1. Resolve default/current branches and checkout ownership. Use a work branch; isolate concurrent work and preserve unrelated changes.
...
6. Handle current feedback under existing authorization. Create required follow-up Issues for independent non-blockers. A push or queue receipt is not resolution. Keep Monitor and Review attached to this exact PR; waiting invokes no model. Respect cumulative review limits and human-owned gates. When the user authorizes a fresh review, invoke Review's `continue --decision-file` with that decision: old phase or failed publication does not block a new independent session. Do not manually edit its state or supply an approval.
7. Under existing merge authorization, recheck current head-bound approval, unchanged gates, blocking feedback and mergeability, then merge and invoke Monitor's protected completion. The helper composes these steps without bypassing protections. Preserve unsafe/unproven cleanup and closed-unmerged local work.
