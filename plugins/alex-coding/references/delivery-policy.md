...
A Delivery Contract governs one identified delivery from its accepted planning handoff through implementation and required review. Contract-backed delivery is appropriate when a large scope, redesign, refactor or materially unsettled outcome needs acceptance frozen first. Small settled logic changes, visual refinements and interaction refinements may use direct implementation acceptance instead; they do not require a Plan merely because they change behavior. Only a contract or settled direct acceptance explicitly bound to the current task or pull request supplies its acceptance criteria. When that delivery is accepted and merged, or explicitly abandoned, the contract becomes an unchanged historical record. Later work may consult it for context but does not reconcile with it, amend it or treat it as a standing product specification. A later accepted behavior change is not a regression merely because it differs from a completed contract; persistent product rules must live in the project's current authoritative sources.
